...
const transformColumn = (
  ...
  if (column.isNullable) {
    args.push(new IdentifierNode('z.null()'));
  }
...
const transformColumnToArgs = (
  ...
  if (enumValues) {
    ...
    const symbolName = context.symbols.set(symbolId, {
      node: enumerate(transformEnum(enumValues)),
      type: SymbolType.DEFINITION,
    });
...
const enumerate = (args: ExpressionNode[]) => {
  switch (args.length) {
    case 0:
      return new IdentifierNode('never');
    default:
      return new EnumExpressionNode(args);
  }
};

export const zodTransform = (options: TransformOptions) => {
